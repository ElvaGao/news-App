...
var createFragment = function( loc, ul, index, reverse) { //拼接列表内容	
	/*所需变量*/
	var title = "Shocking! You Actually Do Harm To Your Dog When Feeding Him 100% Meat";//标题
	var website = "LittleFries";//来源网站
	var comment_number = 42;//评论数量
	/*图片的src*/
	var img_src = "images/dog5.jpg";//一张大图片 和 一张GIF图片 和 一张小图片
	var img_src_small1 = "images/dog3.jpg";//三张小图片（1）
	var img_src_small2 = "images/dog6.jpg";//三张小图片（2）
	var img_src_small3 = "images/dog7.jpg";//三张小图片（3）
	var video_src = "";//video
//	switch(li类型){
//		case 一张大图片／一张GIF图片:
//			img_src = "#";
...
mui(".mui-slider-group").on("tap","li",function(){
	...
	if(mui.os.ios){ //ios
		openDetail("articleIos.html",newHref);
	}else{ //安卓
		openDetail("articleAndroid.html",newHref);
	}
